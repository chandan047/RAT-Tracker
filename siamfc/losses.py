...
__all__ = ['BalancedLoss', 'FocalLoss', 'GHMCLoss', 'OHNMLoss', 'MarginLoss']
...
class MarginLoss(nn.Module):
    ...
    def forward(self, input, targets, size_average=False):
        pos_mask = (targets == 1)
        neg_mask = (targets == 0)
        pos_num = pos_mask.sum().float()
        neg_num = neg_mask.sum().float()
        losses = targets.float() * F.relu(self.m_pos - input).pow(2) / pos_num + \
                 self.lambda_ * (1. - targets.float()) * F.relu(input - self.m_neg).pow(2) / neg_num
